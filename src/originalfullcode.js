// dit is de volleige code  die we moeten verdelen over de modules en zorgen dat het werkt://
...
function updateMapState(activeCategories = []) {
  ...
  document.querySelectorAll('.filter-btn').forEach((button) => {
    const { category } = button.dataset; // HOOFDLETTERS verwacht
    if (category) {
...
class GeolocationManager {
  ...
  showBoundaryPopup() {
    ...
    button.addEventListener('click', function () {
      // Use function() to get correct 'this' for the button if needed, or use arrow function if 'self' is enough
      ...
      map.flyTo({
        // Assuming map is accessible here, otherwise pass it or use self.map
        center: CONFIG.MAP.center,
        zoom: finalZoom,
        pitch: 55,
        bearing: -17.6,
...
function applyMapFilters() {
  ...
  if (activeFilters.size === 0) {
    ...
    // Combineer actieve filters MET markers zonder categorie
    filterExpression = [
      'any', // OF-conditie
      ['in', ['get', 'category'], ['literal', Array.from(activeFilters)]], // Markers met actieve categorieën
      ['!', ['has', 'category']], // Markers zonder categorie property
      ['==', ['get', 'category'], ''], // Markers met lege categorie
    ];
  }
...
map.on('load', () => {
  ...
  map.once('idle', () => {
    ...
    map.addLayer(
      ...
      firstSymbolLayerId
    ); // Belangrijk: plaats vóór de eerste symboollaag
  });
